...
if __name__ == "__main__":
    main()
Добавь к  коду возможность отправлять жалобы по сессиям телеграм и была еще одна возможность(если пользователь выбрал снос через телеграм) -  отправлять жалобы через телеграм с использованием прокси( нужно будет создать прокси файл и туда их поместить, не забудь это упомянуть в выводе кода)или отправлять жалобы через телеграм, но без прокси( добавь, упоминанание, что таким образом рекомендуется отправлять жалобы не более чем с 10 аккаунтов, иначе телеграм убьет сессии). Также убери блок кода с прокси(пусть прокси будут хранится в отдельном месте) и блок с почтами(почты тоже будут хранится в файле). Сессии тоже будут хранится в файле в формате session. Как изменишь код, подробно расскажи, что делает этот код и как с ним работать
